  ...
  class AddressDraft
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `CommercetoolsSdk::AddressDraft` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `CommercetoolsSdk::AddressDraft`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
    ...
    def hash
      [id, key, country, title, salutation, first_name, last_name, street_name, street_number, additional_street_info, postal_code, city, region, state, company, department, building, apartment, p_o_box, phone, mobile, email, fax, additional_address_info, external_id, custom].hash
    end
