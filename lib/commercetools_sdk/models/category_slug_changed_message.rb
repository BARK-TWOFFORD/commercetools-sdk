...
module CommercetoolsSdk
  class CategorySlugChangedMessage
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `CommercetoolsSdk::CategorySlugChangedMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `CommercetoolsSdk::CategorySlugChangedMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      if attributes.key?(:old_slug)
        self.old_slug = attributes[:old_slug]
      end
    end
    ...
    def hash
      [id, version, created_at, last_modified_at, last_modified_by, created_by, sequence_number, resource, resource_version, type, resource_user_provided_identifiers, slug, old_slug].hash
    end
